function assemble (code) {
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    switch (statements[0]) {
      ...
      case 'const': {
      	if (statements[1] in vars) {
        	alert(`'${statements[1]}' already exists as a label or const.`);
          return false;
        }
        if (isNaN(Number(statements[2]))) {
        	alert(`'${statements[2]}' cannot be parsed as a number.`);
          return false;
        }
      ...
      case 'ACSET': {
          ...
          if (isNaN(value)) {
            alert('Syntax error on line ' + i + ': ' + lines[i]);
            return false;
          }
